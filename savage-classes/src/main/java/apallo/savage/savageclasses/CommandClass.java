...
public class CommandClass implements CommandExecutor, TabCompleter  {
        ...
        if(sender instanceof Player) {
            player = (Player) sender;
                ...
                if(args.length == 0 && sender instanceof Player){
                    ...
                    if(args[0].equalsIgnoreCase("reload") && args.length == 1 && sender.hasPermission("savageclasses.class.reload")){
                        SC.reloadClassStatsConfig();
                        SavageUtility.displayMessage("Classes config reloaded", player);
                        return true;
                    }
                    else if(args.length == 3 && args[0].equalsIgnoreCase("change") && sender.hasPermission("savageclasses.class.change.others")){
                        SavageUtility.displayMessage(ChatColor.DARK_GREEN + "You successfully changed classes", Bukkit.getPlayer(args[1]));
                        SC.setPlayerClass(args[1], args[2]);
                        return true;
                    }
                        ...
                        else if (args[0].equalsIgnoreCase("shaman") && args.length == 1) {
                            SavageUtility.displayMessage("You are now a shaman.", player);
                            SC.setPlayerClass(player, "shaman");
                        }
                        else if (args[0].equalsIgnoreCase("chaoscrusader") && args.length == 1) {
                            SavageUtility.displayMessage("You are now a chaos crusader.", player);
                            SC.setPlayerClass(player, "chaoscrusader");
                        }
                        else if (args[0].equalsIgnoreCase("martyr") && args.length == 1) {
                            SavageUtility.displayMessage("You are now a martyr.", player);
                            SC.setPlayerClass(player, "martyr");
                        }
                        else if (args[0].equalsIgnoreCase("shadowknight") && args.length == 1) {
                            SavageUtility.displayMessage("You are now a shadow knight.", player);
                            SC.setPlayerClass(player, "shadowknight");
                        }
                        else if (args[0].equalsIgnoreCase("warlock") && args.length == 1) {
                            SavageUtility.displayMessage("You are now a warlock.", player);
                            SC.setPlayerClass(player, "warlock");
                        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if(sender.hasPermission("savageclasses.class.change")) {
                ...
                list.add("berserker");
                list.add("shaman");
                list.add("chaoscrusader");
                list.add("martyr");
                list.add("shadowknight");
                list.add("warlock");
