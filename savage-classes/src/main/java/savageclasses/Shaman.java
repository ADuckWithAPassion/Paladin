...
import net.minecraft.server.v1_14_R1.CircularTimer;
import net.minecraft.server.v1_14_R1.SensorDummy;
...
implements Listener
{
	...
	private void curse(Player player) {
		Player target = (Player) SavageUtility.getInfront(player,curseRange, Player.class, "notInParty");
		if(target == null) {
		...
		for(Entity nearby: SavageUtility.getPlayersWithin(target.getLocation(), curseAOE)) {
			if(!(SavageUtility.getPartyMembers(player).contains((Player)nearby)) || player==(Player)nearby) {
				...
				affected.playSound(affected.getLocation(), Sound.BLOCK_LAVA_EXTINGUISH, 25, 25);
			}
		}
				...
				for(Player nearby: totem.getWorld().getEntitiesByClass(Player.class)) {
					if(nearby.getLocation().distance(totem.getLocation()) <= healthTotemRange) {
						...
						if(SavageUtility.getPartyMembers(player).contains(nearby)) {
							...
							double maxHealth = nearby.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue(); // non depreciated way of getting max health
							nearby.setHealth(Math.min(maxHealth, health)); // no overflow
							nearby.sendMessage(ChatColor.DARK_GRAY + "[SR] " + ChatColor.GOLD + player.getName()+"'s " + ChatColor.AQUA + "Healing Totem" + ChatColor.AQUA + " renews you.");
							nearby.playSound(nearby.getLocation(), Sound.AMBIENT_UNDERWATER_EXIT, 25, 25);
							nearby.getWorld().spawnParticle(Particle.HEART, nearby.getLocation().add(0, 2, 0), 1, 0, 0, 0);
						}
					}
